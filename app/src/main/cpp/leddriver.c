...
int ledDriver_Init()
{
    ...
    if (retVal < 0)
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "MyTag", "Failed to read device ID");

    }
    else
    {
        printf("\n i2cRead Success: device id : 0x%x",data);
        __android_log_print(ANDROID_LOG_INFO, "MyTag", " \n i2cRead Success: device id : 0x%x",data);
    ...
    if (retVal < 0)
    {
        ...
        printf("\n Failed to read revision ID changes by deepak\n");
        __android_log_print(ANDROID_LOG_INFO, "MyTag", " \n Failed to read revision ID\n");
...
//        __android_log_print(ANDROID_LOG_INFO, "MyTag", "Failed to enable fast-softstart\n");
//
//        return I2C_FAILURE;
//    }
    return  SUCCESS;
